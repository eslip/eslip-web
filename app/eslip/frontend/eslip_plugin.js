...
function isAutoInit(){
    var eslipScript = document.getElementById("eslip_script");
    var autoInit = 'true';
    if (eslipScript && typeof eslipScript !== 'undefined'){
        autoInit = eslipScript.getAttribute("autoInit") || 'true';
    }
    return autoInit;
}
...
function createWidget(){
    ...
    ajax.open("GET", pluginURL+"eslip_frontend_services/getWidgetData", true);
...
function clickLogin(server){
    ...
    if (server == 'openid'){
        var url = pluginURL+"eslip_openid.php?referer="+encodeURIComponent(referer);
    }
    else{
        var url = pluginURL+"eslip_oauth.php?server="+server+"&referer="+encodeURIComponent(referer);    
    }
    openLoginWindow(url);
}
